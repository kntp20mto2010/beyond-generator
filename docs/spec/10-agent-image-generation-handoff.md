...
6. `wait_image_request`で`completed`、`failed`、`timeout`のいずれかを確認する。
7. `completed`なら画像ファイルの存在を確認してから実装へ組み込む。
8. `failed`または`timeout`なら、同じIDを再利用せず原因を確認して新IDで再依頼する。
...
依頼側は対象IDを保持し、必ず`wait_image_request`または`get_image_request`で
`completed`または`failed`を確認する。通常は`wait_image_request`を使う。
`wait_image_request`は未claimのpendingを1回だけ再wakeできる。
...
#### 抽出物の輪郭クリーン (navy 縁被りの除去) は Codex に任せる (2026-06-24, KEN 方針)

`apply-green-mask` は部屋の原画ピクセルをそのまま切り出すので、**家具の輪郭に壁色 (navy 等) が
アンチエイリアスで焼き込まれた「縁被り」が残る**。フラットデザインでは意図しない暗い縁取り線になって
致命的 (実測: navy 観葉植物 正面で輪郭画素の **59%** が navy 被り)。

これは決定論スクリプト (erode/despill/アンマット) では細い有機形状で取り切れない (defringe で 28% 止まり)。
**KEN 方針 = Codex に輪郭を綺麗にさせる**。手順 (上の単色クロマキーと一体):

1. 抽出物をマゼンタ #FF00FF にパディング付きで合成 (`onmagenta` 入力)。
2. `kind: "edit"` で「**輪郭の navy 縁被りだけを除去し、フチを本来の家具色のクリーンなフラット縁にする。
   背景はマゼンタのまま。形・色・内部は一切変えない (内部を描き直さない)**」と依頼。
3. `wait_image_request` → completed を Read で目視 (縁が消えたか・ドリフトしてないか・背景一様マゼンタか)。
4. `key-chroma-bg.py --color magenta --tight-crop` で抜く。
5. `objects/` を差替 + catalog の `promptFile` に edgeclean を記録、寸法を更新。
- 実証 (2026-06-24): navy 植物 正面で **navy 縁 59%→4%**、形/葉/色を保持。決定論 (28%) を大きく上回る。
- コスト: 1 家具 = Codex round-trip 1 回。**縁被りが目立つ抽出物に適用**する (clean 抽出でも輪郭が命なので)。
...
古い依頼が不要になった場合は`cancel_image_request`で落とす。新しい依頼で
置き換えた場合は`supersede_image_request`を使い、古いpendingを残さない。
